...
///
/// ⚠️ <b>방어 수준을 정직하게 적어 둔다(재검수 R-1).</b>
///  · <b>인터넷이 끊긴 동안에는 이 파일이 유일한 방어다.</b> 서버가 센 값은 "그 날짜가 오늘일 때만" 쓰는데,
///    오프라인이면 그 날짜가 갱신되지 않으므로 사실상 쓰이지 않는다.
///  · 따라서 이 파일과 대기줄을 <b>둘 다 지우고 앱을 껐다 켜면</b> 오프라인 한도는 되살아난다.
///    이것은 "작업관리자로 앱을 끄면 잠금이 풀린다"(지시서 §0에서 수용한 한계)와 같은 급의 문제이고,
///    설치형 앱만으로는 막을 수 없다. <b>온라인으로 돌아온 뒤에는</b> 서버가 센 값이 다시 막는다.
///  · 근본 대응은 서버 쪽 감지(오프라인 사용이 허용량을 넘은 PC를 [PC관리]에 표시)다 — <b>미구현·사장님 결정 대기</b>.
/// </summary>
