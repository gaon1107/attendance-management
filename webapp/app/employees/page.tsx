...
export default async function EmployeesPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", background: "var(--bg)" }}>
      <Sidebar user={me} active="employees" />
      ...
      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 18, fontWeight: 700 }}>직원 관리</span>
            <span style={{ fontSize: 13, color: "#9CA3AF" }}>
              {me.company.name} · 총 {employees.length}명
            </span>
          </div>
        </header>
        ...
        <div style={{ flex: 1, minWidth: 0, padding: "20px 28px 40px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 16 }}>
            {kpis.map((k) => (
              ...
              >
                <div style={{ fontSize: 13, color: "var(--text-sub)", fontWeight: 700, marginBottom: 10, whiteSpace: "nowrap" }}>
                  {k.label}
                </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {employees.length === 0 ? (
                  ...
                  employees.map((emp) => {
                    const auth =
                      emp.authMethod === "face" ? "얼굴인증" : emp.authMethod === "gps" ? "GPS" : "미설정";
                    ...
                    return (
                      <tr key={emp.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                        ...
                        <td style={{ ...td, color: consented ? "#15803D" : "#9CA3AF" }}>
                          {consented ? "동의함" : "—"}
                        </td>
                        <td style={{ ...td, color: "var(--text-sub)", fontVariantNumeric: "tabular-nums" }}>
                          {ymd(emp.createdAt)}
                        </td>
                        <td style={td}>
                          <span
