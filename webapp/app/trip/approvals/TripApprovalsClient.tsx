"use client";
// 출장 승인 목록 (클라이언트) — 공통 기간 달력 + 통합검색. (재택 승인 패턴 + 출장지)
...
export function TripApprovalsClient({
  pending,
  decided,
  from,
  to,
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        <RangeCalendarNav from={from} to={to} todayISO={todayISO} basePath="/trip/approvals" />
        <SearchBox value={q} onChange={setQ} placeholder="신청자·출장지·사유 검색" />
      </div>
              ...
              {pgP.view.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: "28px 20px", fontSize: 14, color: "var(--text-sub)", textAlign: "center" }}>
                    {q.trim() ? "검색 결과가 없습니다." : "승인 대기 중인 출장 신청이 없습니다."}
                  </td>
                </tr>
              ) : (
                pgP.view.map((r) => (
                  <tr key={r.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                    ...
                    <td style={{ ...td, color: "var(--text-sub)", maxWidth: 260 }}>
              ...
              {pgD.view.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: "24px 20px", fontSize: 14, color: "var(--text-sub)", textAlign: "center" }}>
                    {q.trim() ? "검색 결과가 없습니다." : "이 기간에 처리한 출장이 없습니다."}
                  </td>
                </tr>
              ) : (
                pgD.view.map((r) => {
                  ...
                  return (
                    <tr key={r.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                      ...
                      <td style={{ ...td, color: "var(--text-sub)", fontVariantNumeric: "tabular-nums" }}>{r.employeeNo || "—"}</td>
                      <td style={{ ...td, color: "var(--text-sub)", fontVariantNumeric: "tabular-nums" }}>{r.rangeLabel}</td>
                      <td style={{ ...td, color: "var(--text-sub)", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.destination}</td>
                      <td style={td}>
