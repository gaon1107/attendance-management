...
import {
  parseTempReasons, isTempUseType, TEMP_USE_TYPE, TEMP_USE_OFFLINE_TYPE,
  EVENT_MAX_AGE_DAYS, EVENT_FUTURE_TOLERANCE_MIN,
} from "@/lib/pcoff";
...
export async function POST(req: Request) {
  ...
  for (const item of raw) {
    ...
    //  · 미래 한계도 같은 이유로 넉넉하다(EVENT_FUTURE_TOLERANCE_MIN). 한 달간 시계를 못 맞춘 PC는
    //    몇 분 앞서 있을 수 있는데, 그 오차로 한 달치를 버리면 안 된다.
    const at = new Date(String(e.at ?? ""));
    if (Number.isNaN(at.getTime())) { dropped++; continue; }
    const diff = at.getTime() - now;
    if (diff > EVENT_FUTURE_TOLERANCE_MIN * 60 * 1000 || diff < -EVENT_MAX_AGE_DAYS * 24 * 60 * 60 * 1000) { dropped++; continue; }
