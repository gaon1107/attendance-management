"use client";
// 초과근무 승인 목록 (클라이언트) — 공통 기간 달력 + 통합검색. (외출외근 승인 패턴)
...
export function OvertimeApprovalsClient({
  pending,
  decided,
  from,
  to,
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        <RangeCalendarNav from={from} to={to} todayISO={todayISO} basePath="/overtime/approvals" />
              ...
              {pgP.view.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ padding: "28px 20px", fontSize: 14, color: "var(--text-sub)", textAlign: "center" }}>
                    {q.trim() ? "검색 결과가 없습니다." : "승인 대기 중인 초과근무 신청이 없습니다."}
                  </td>
                </tr>
              ) : (
                pgP.view.map((r) => (
                  <tr key={r.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                    ...
                    <td style={{ ...td, textAlign: "right" }}>
                      <div style={{ display: "inline-flex", gap: 8 }}>
                        <ConfirmApproveButton action={approveOvertime} requestId={r.id} progress={r.progress} compact />
                        <RejectButton action={rejectOvertime} requestId={r.id} compact />
                      </div>
                    </td>
                  </tr>
                ))
              ...
              {pgD.view.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: "24px 20px", fontSize: 14, color: "var(--text-sub)", textAlign: "center" }}>
                    {q.trim() ? "검색 결과가 없습니다." : "이 기간에 처리한 초과근무가 없습니다."}
                  </td>
                </tr>
              ) : (
                pgD.view.map((r) => {
