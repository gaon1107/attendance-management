...
export function ApprovalHistoryFilters({
  ...
  // 현재 값에 변경분(over)만 덮어 URL을 만든다. 기본값(all/기간 이번달)은 파라미터 생략 가능하지만
  // 기간은 항상 유지해야 하므로 from/to는 늘 싣는다.
  ...
  return (
    <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: 16, marginBottom: 16, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
      <label style={{ display: "flex", flexDirection: "column", gap: 4, ...capStyle }}>
        유형
        <select defaultValue={type} onChange={(e) => router.push(build({ type: e.target.value }))} style={selectStyle}>
      ...
      <label style={{ display: "flex", flexDirection: "column", gap: 4, ...capStyle }}>
        상태
        <select defaultValue={status} onChange={(e) => router.push(build({ status: e.target.value }))} style={selectStyle}>
      ...
      <label style={{ display: "flex", flexDirection: "column", gap: 4, ...capStyle }}>
        직원
        <select defaultValue={userId} onChange={(e) => router.push(build({ userId: e.target.value }))} style={{ ...selectStyle, maxWidth: 220 }}>
          <option value="all">전체</option>
