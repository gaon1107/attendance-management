// 인증방식 선택 (직원) — 얼굴 / GPS. 얼굴은 강제가 아니며 언제든 바꿀 수 있음(인권위 기준).
...
export default async function AuthMethodPage({
  ...
  const consentDate = me.faceConsentAt
    ? new Date(me.faceConsentAt).toLocaleDateString("ko-KR")
    : null;
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      ...
      <main style={{ maxWidth: 560, margin: "0 auto", padding: "28px 24px" }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>출퇴근 인증방식</h1>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px", marginBottom: 20, fontSize: 14 }}>
          ...
          {method === "face" && consentDate && (
            <span style={{ color: "var(--text-sub)" }}> · 생체정보 동의일 {consentDate}</span>
          )}
        </div>
