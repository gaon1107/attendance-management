...
export default async function AttendancePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", background: "var(--bg)" }}>
      <Sidebar user={me} active="attendance" />
      ...
      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            <span style={{ fontSize: 18, fontWeight: 700, whiteSpace: "nowrap" }}>내 출퇴근</span>
            <span style={{ fontSize: 13, color: "#9CA3AF", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {me.name} 님
            </span>
          </div>
          <span style={{ fontSize: 13, color: "var(--text-sub)", whiteSpace: "nowrap" }}>{todayLabel}</span>
        </header>

        {/* 가운데 정렬 유동 폭 — PC에선 중앙, 모바일에선 꽉 참 */}
        <div style={{ flex: 1, minWidth: 0, padding: "24px 16px 48px" }}>
            ...
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 24 }}>
              ...
              >
                ...
                <div style={{ textAlign: "center", flex: 1 }}>
                  ...
                  <div style={{ fontSize: 18, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: latest?.clockOut ? "var(--text)" : "#9CA3AF" }}>
                    {outLabel}
                  </div>
                </div>
              </div>
                ...
                <>
                  ...
                  <form action={startBreak} style={{ display: "flex", gap: 8 }}>
                    <select
                      name="reason"
                      defaultValue="식사"
                      style={{ flex: 1, height: 48, padding: "0 12px", border: "1px solid #D1D5DB", borderRadius: 10, fontFamily: "inherit", fontSize: 15, background: "#fff" }}
                    >
                      {REASONS.map((r) => (
                    ...
                    <button
                      type="submit"
                      style={{ width: 110, height: 48, border: "1px solid var(--border)", borderRadius: 10, background: "#fff", color: "var(--text)", fontFamily: "inherit", fontSize: 15, fontWeight: 700, cursor: "pointer" }}
                    >
                      외출
                    </button>
                  </form>
              ...
              {todays.length === 0 ? (
                ...
                todays.map((rec) => {
                  ...
                  return (
                    <div
                      key={rec.id}
                      style={{ padding: "14px 16px", background: "var(--card)", border: "1px solid var(--border)", borderRadius: 11, marginBottom: 8, fontSize: 14 }}
                    >
