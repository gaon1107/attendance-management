"use client";
// 재택근무 승인 목록 (클라이언트) — 공통 기간 달력 + 통합검색. (휴가 LeaveApprovalsClient 패턴)
...
export function RemoteApprovalsClient({
  pending,
  decided,
  from,
  to,
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        <RangeCalendarNav from={from} to={to} todayISO={todayISO} basePath="/remote/approvals" />
                  ...
                  <tr key={r.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                    ...
                    <td style={{ ...td, textAlign: "right" }}>
                      <div style={{ display: "inline-flex", gap: 8 }}>
                        <ConfirmApproveButton action={approveRemote} requestId={r.id} progress={r.progress} compact />
                        <RejectButton action={rejectRemote} requestId={r.id} compact />
                      </div>
                    </td>
                  </tr>
                ))
