// 재택근무(직원 본인) — 신청 폼 + 내 신청 내역. (휴가 화면과 같은 형식)
...
export default async function RemotePage() {
  ...
  const progressMap: Map<string, ApprovalProgress> =
    (me.company.approvalMode === "deptline" || me.company.approvalMode === "custom")
      ? await getApprovalProgressMap(me.companyId, "remote", requests.filter((r) => r.status === "pending").map((r) => r.id))
      : new Map();
  ...
  return (
    <AppShell user={me} active="remote" title="재택근무" subtitle={`${me.name} 님`}>
      ...
      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: 24 }}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>재택근무 신청</div>
        <p style={{ fontSize: 13, color: "var(--text-sub)", marginBottom: 18, lineHeight: 1.6 }}>
          재택근무할 기간을 미리 신청해 승인받는 기능입니다. 승인은 <b>사전 허가 기록</b>으로 남으며, 출퇴근·실근무 계산은 기존 방식(출근 시 재택 선택) 그대로입니다.
        </p>
        <RemoteRequestForm />
      </div>
                    ...
                    <tr key={r.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                      ...
                      <td style={td}>
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, height: 24, padding: "0 9px", borderRadius: 6, background: s.bg }}>
                          ...
                          <span style={{ fontSize: 13, fontWeight: 700, color: s.color }}>{remoteStatusLabel(r.status)}</span>
                        </span>
                        {(() => {
