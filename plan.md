...
## 4. 법적 안전장치 (⚠️ 사장님 확인 필요 항목 포함)
- 표기 문구: **"본인 확인 재검토 필요"**(단정 금지) — 기준값 미만일 때만 배지. 직원 화면 표시 없음.
- 사진 보호: AES-256 암호화, 웹 공개경로 밖 저장, 관리자만 열람 + 열람 기록, 보관기한 지나면 자동 파기.
- ❓ **확인 1 — 저장 범위**: 기존 확정(7/10)은 "의심건만 저장"이었는데 이번 지시는 "출퇴근 사진 전건 이력 저장"으로 이해했습니다. **전건 저장으로 계획했습니다** — 맞습니까? (전건 저장은 생체정보 보관이 늘어나 동의서에 반드시 명시해야 합니다)
- ❓ **확인 2 — 보관기간**: 기존 확정은 90일 자동 파기. 전건 저장에도 **90일 적용을 권장**합니다(근태 시각 기록 자체는 3년 보존되고, 사진만 파기). 다르게 원하시면 알려주세요.
- 동의서·face-spec 문구 갱신은 구현 TODO에 포함(전문가 검토 목록 유지).
...
- [ ] 8) 동의서/face-spec 문구에 "출퇴근 사진 보관·파기" 반영 — 파일: docs/07_ai/face-spec.md, consent 문구
