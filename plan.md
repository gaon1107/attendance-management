...
- [x] 4-a: `settings.ts` 5곳에 `config` 기록(add-only) — 커밋 `b2ace5f`
- [x] 4-b: `authmethod.ts` 3곳에 `purge` 기록(redirect 순서 지킴) — 커밋 `b2ace5f`
- [x] 4-c: 접속 로그 화면·엑셀에 "설정·파기" 필터 추가 — 커밋 `b2ace5f`
- [x] 4-d: 회귀 테스트(실행 증거 확보 — 커밋 메시지 참조)
- [x] 4-e: code-reviewer 검수 → **치명0·중간6·경미 다수 전부 반영** — 커밋 `7999e4b`

### 검수 결과 요약 (code-reviewer, 2026-07-16)
- **치명 0**. 집중 점검 요청 항목(redirect 함정·성공지점만 기록·라벨 파싱 엣지케이스 15종·화면/엑셀 규칙 일치·회사격리·3단계 회귀) **전부 통과**.
- **중간 6 → 전부 반영**: ①가짜 파기 기록 ②실패인데 성공으로 기록 ③data_view 침묵누락 ④동명이인 구분불가 ⑤after() 미사용 ⑥동의 개시 미기록(5단계 백로그)
- 뿌리는 하나 — **"감사 로그가 사실과 다르게 말하면 안 된다"**. 이 기능의 존재 이유가 정확히 그 신뢰성이므로 ①②를 최우선 수정.
